...
REGX_SYMBOL = r"(?P<symbol>[a-zA-Z_.:$][a-zA-Z_.:$0-9]*)"
REGX_LABEL = r"^\("+REGX_SYMBOL+"\)$"
REGX_SYMBOLIC_A = r"^@"+REGX_SYMBOL+"$"
...
def resolveSymbols(lines):
    ...
    for i in xrange(len(lines)):
        line = lines[i][0]
        m = re.match(REGX_SYMBOLIC_A, line)
        if m:
